...
{
	ImAccountsService *service = IM_ACCOUNTS_SERVICE(object);
	ImAccountsServicePrivate * priv = im_accounts_service_get_instance_private(service);
...
user_changed (ActUserManager * manager, ActUser * user, gpointer user_data)
{
	...
	ImAccountsServicePrivate * priv = im_accounts_service_get_instance_private(user_data);
...
im_accounts_service_set_draws_attention (ImAccountsService * service, gboolean draws_attention)
{
	...
	g_dbus_connection_call(g_dbus_proxy_get_connection(priv->touch_settings),
		...
		"Set",
		g_variant_new("(ssv)", "org.freedesktop.DisplayManager.AccountsService", "HasMessages", g_variant_new_boolean (draws_attention)),
		NULL, /* reply */
