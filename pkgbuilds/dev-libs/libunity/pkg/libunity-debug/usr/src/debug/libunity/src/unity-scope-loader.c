/* unity-scope-loader.c generated by valac 0.56.19, the Vala compiler
...
static gint UnityScopeLoader_private_offset;
...
static GType unity_scope_loader_get_type_once (void);
static inline gpointer _vala_memdup2 (gconstpointer mem,
                        gsize byte_size);

static inline gpointer
unity_scope_loader_get_instance_private (UnityScopeLoader* self)
{
	return G_STRUCT_MEMBER_P (self, UnityScopeLoader_private_offset);
}
                                    ...
                                    GError** error)
{
	...
	_tmp30_ = load_scopes;
	_tmp31_ = _tmp30_ (&_inner_error0_);
	_tmp29_ = _tmp31_;
...
unity_scope_loader_get_scopes (UnityScopeLoader* self,
                               ...
                               GError** error)
{
	UnityScopeLoaderClass* _klass_;
	g_return_val_if_fail (self != NULL, NULL);
	_klass_ = UNITY_SCOPE_LOADER_GET_CLASS (self);
	if (_klass_->get_scopes) {
		return _klass_->get_scopes (self, module_name, module_type, error);
	}
	return NULL;
}
	...
	GError* _inner_error0_ = NULL;
	{
		...
		for (scope_it = scope_collection; scope_it != NULL; scope_it = scope_it->next) {
			...
			scope = _tmp0_;
			{
				...
				connector = _tmp2_;
				_tmp3_ = connector;
				unity_scope_dbus_connector_export (_tmp3_, &_inner_error0_);
				if (G_UNLIKELY (_inner_error0_ != NULL)) {
					g_propagate_error (error, _inner_error0_);
                                  ...
                                  GError** error)
{
	...
	_klass_ = UNITY_SCOPE_LOADER_GET_CLASS (self);
	if (_klass_->export_scopes) {
		_klass_->export_scopes (self, scopes, error);
	}
}
                                    ...
                                    GError** error)
{
	...
	_tmp5_ = unity_scope_loader_get_scopes (self, module, module_type, &_inner_error0_);
	...
	unity_scope_loader_export_scopes (self, _tmp6_, &_inner_error0_);
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
		g_propagate_error (error, _inner_error0_);
                               ...
                               GError** error)
{
	...
	UnityProtocolScopeGroupConfig* _tmp2_;
	GList* _tmp3_;
	GError* _inner_error0_ = NULL;
	g_return_if_fail (self != NULL);
	g_return_if_fail (group_name != NULL);
	_tmp0_ = unity_protocol_scope_group_config_new (group_name, &_inner_error0_);
	config = _tmp0_;
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
		g_propagate_error (error, _inner_error0_);
		return;
	...
	_tmp1_ = config;
	unity_internal_update_process_timeout (_tmp1_->timeout);
	_tmp2_ = config;
	_tmp3_ = _tmp2_->scopes;
	{
		GList* info_collection = NULL;
		GList* info_it = NULL;
		info_collection = _tmp3_;
		for (info_it = info_collection; info_it != NULL; info_it = info_it->next) {
			UnityProtocolScopeGroupScopeInfo* _tmp4_;
			UnityProtocolScopeGroupScopeInfo* info = NULL;
			_tmp4_ = __unity_protocol_scope_group_scope_info_dup0 ((UnityProtocolScopeGroupScopeInfo*) info_it->data);
			info = _tmp4_;
			{
				UnityProtocolScopeGroupScopeInfo* _tmp5_;
				const gchar* _tmp6_;
				UnityProtocolScopeGroupScopeInfo* _tmp7_;
				const gchar* _tmp8_;
				_tmp5_ = info;
				_tmp6_ = (*_tmp5_).module;
				_tmp7_ = info;
				_tmp8_ = (*_tmp7_).module_type;
				unity_scope_loader_load_and_export (self, _tmp6_, _tmp8_, &_inner_error0_);
				if (G_UNLIKELY (_inner_error0_ != NULL)) {
					g_propagate_error (error, _inner_error0_);
                               ...
                               GError** error)
{
	...
	_tmp0_ = unity_protocol_scope_registry_scope_metadata_for_id (scope_id, &_inner_error0_);
	metadata = _tmp0_;
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
		g_propagate_error (error, _inner_error0_);
		return;
	...
	_tmp4_ = metadata;
	_tmp5_ = _tmp4_->module_type;
	unity_scope_loader_load_and_export (self, _tmp3_, _tmp5_, &_inner_error0_);
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
		g_propagate_error (error, _inner_error0_);
                                ...
                                GError** error)
{
	...
	g_return_if_fail (self != NULL);
	g_return_if_fail (module != NULL);
	unity_scope_loader_load_and_export (self, module, module_type, &_inner_error0_);
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
		g_propagate_error (error, _inner_error0_);
		return;
	}
                               ...
                               gpointer klass_data)
{
	...
	g_type_class_adjust_private_offset (klass, &UnityScopeLoader_private_offset);
...
unity_scope_loader_instance_init (UnityScopeLoader * self,
                                  gpointer klass)
{
	self->priv = unity_scope_loader_get_instance_private (self);
}
...
static GType
unity_scope_loader_get_type_once (void)
{
	...
	UnityScopeLoader_private_offset = g_type_add_instance_private (unity_scope_loader_type_id, sizeof (UnityScopeLoaderPrivate));
	return unity_scope_loader_type_id;
}

GType
unity_scope_loader_get_type (void)
{
	static volatile gsize unity_scope_loader_type_id__once = 0;
	if (g_once_init_enter (&unity_scope_loader_type_id__once)) {
		GType unity_scope_loader_type_id;
		unity_scope_loader_type_id = unity_scope_loader_get_type_once ();
		g_once_init_leave (&unity_scope_loader_type_id__once, unity_scope_loader_type_id);
	}
	return unity_scope_loader_type_id__once;
}
