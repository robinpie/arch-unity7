/* music-track-model-renderer.c generated by valac 0.56.19, the Vala compiler
...
static gint UnityTesterMusicTrackModelRenderer_private_offset;
...
VALA_EXTERN DeeModel* unity_tester_music_track_model_renderer_get_track_model (UnityTesterMusicTrackModelRenderer* self);
...
static GType unity_tester_music_track_model_renderer_get_type_once (void);
...
unity_tester_music_track_model_renderer_get_instance_private (UnityTesterMusicTrackModelRenderer* self)
{
	return G_STRUCT_MEMBER_P (self, UnityTesterMusicTrackModelRenderer_private_offset);
}
                                                        ...
                                                        DeeModelIter* iter)
{
	...
	_tmp3_ = self->priv->_track_model;
	_tmp4_ = iter;
	_tmp5_ = self->priv->row_counter;
	self->priv->row_counter = _tmp5_ + 1;
	dee_model_set_tag (_tmp3_, _tmp4_, _tmp2_, (gpointer) ((gintptr) _tmp5_));
	_tmp6_ = self->priv->_track_view_model;
	gtk_list_store_append (_tmp6_, &_tmp7_);
	tm_iter = _tmp7_;
	_tmp8_ = self->priv->_track_view_model;
	_tmp9_ = tm_iter;
	_tmp10_ = row[0];
	...
	_tmp16_ = row[4];
	_tmp17_ = row[5];
	gtk_list_store_set (_tmp8_, &_tmp9_, 0, _tmp11_, 1, g_variant_get_int32 (_tmp12_), 2, _tmp14_, 3, g_variant_get_uint32 (_tmp15_), 4, g_variant_get_uint32 (_tmp16_), 5, g_variant_get_double (_tmp17_), -1, -1);
                                                          ...
                                                          DeeModelIter* iter)
{
	...
	if (_tmp5_) {
		while (TRUE) {
			...
			_tmp6_ = self->priv->_track_view_model;
			_tmp7_ = gtk_tree_model_iter_next ((GtkTreeModel*) _tmp6_, &tm_iter);
			if (!_tmp7_) {
				break;
			}
			index = index - 1;
		...
		if (index == 0) {
			...
			GVariant* _tmp26_;
			_tmp10_ = _tmp9_ = dee_model_get_row (model, iter, NULL);
			row = _tmp10_;
			row_length1 = _vala_array_length (_tmp9_);
			_row_size_ = row_length1;
			_tmp11_ = self->priv->_track_view_model;
			_tmp12_ = tm_iter;
			_tmp13_ = row;
			...
			_tmp26_ = _tmp25_[5];
			gtk_list_store_set (_tmp11_, &_tmp12_, 0, _tmp15_, 1, g_variant_get_int32 (_tmp17_), 2, _tmp20_, 3, g_variant_get_uint32 (_tmp22_), 4, g_variant_get_uint32 (_tmp24_), 5, g_variant_get_double (_tmp26_), -1, -1);
			row = (_vala_array_free (row, row_length1, (GDestroyNotify) g_variant_unref), NULL);
		} else {
			FILE* _tmp27_;
			_tmp27_ = stderr;
			fprintf (_tmp27_, "can't update row");
		}
	}
}
...
unity_tester_music_track_model_renderer_track_model_synchronized_cb (UnityTesterMusicTrackModelRenderer* self)
{
	...
	g_return_if_fail (self != NULL);
	_tmp0_ = self->priv->_track_model;
	g_signal_handler_disconnect (_tmp0_, self->priv->model_sync_sig_id);
                                                         ...
                                                         DeeModel* value)
{
	DeeModel* old_value;
	g_return_if_fail (self != NULL);
	old_value = unity_tester_music_track_model_renderer_get_track_model (self);
	if (old_value != value) {
		DeeModel* _tmp0_;
                                                              ...
                                                              GtkListStore* value)
{
	...
	g_return_if_fail (self != NULL);
	old_value = unity_tester_music_track_model_renderer_get_track_view_model (self);
	if (old_value != value) {
		GtkListStore* _tmp0_;
		_tmp0_ = _g_object_ref0 (value);
                                                    ...
                                                    gpointer klass_data)
{
	...
	g_type_class_adjust_private_offset (klass, &UnityTesterMusicTrackModelRenderer_private_offset);
...
unity_tester_music_track_model_renderer_instance_init (UnityTesterMusicTrackModelRenderer * self,
                                                       gpointer klass)
{
	self->priv = unity_tester_music_track_model_renderer_get_instance_private (self);
...
static GType
unity_tester_music_track_model_renderer_get_type_once (void)
{
	...
	UnityTesterMusicTrackModelRenderer_private_offset = g_type_add_instance_private (unity_tester_music_track_model_renderer_type_id, sizeof (UnityTesterMusicTrackModelRendererPrivate));
	return unity_tester_music_track_model_renderer_type_id;
}

GType
unity_tester_music_track_model_renderer_get_type (void)
{
	static volatile gsize unity_tester_music_track_model_renderer_type_id__once = 0;
	if (g_once_init_enter (&unity_tester_music_track_model_renderer_type_id__once)) {
		GType unity_tester_music_track_model_renderer_type_id;
		unity_tester_music_track_model_renderer_type_id = unity_tester_music_track_model_renderer_get_type_once ();
		g_once_init_leave (&unity_tester_music_track_model_renderer_type_id__once, unity_tester_music_track_model_renderer_type_id);
	}
	return unity_tester_music_track_model_renderer_type_id__once;
}
