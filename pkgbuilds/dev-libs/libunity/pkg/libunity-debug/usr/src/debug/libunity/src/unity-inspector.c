/* unity-inspector.c generated by valac 0.56.19, the Vala compiler
...
static gint UnityInspector_private_offset;
...
static GType unity_inspector_get_type_once (void);
...
unity_inspector_get_instance_private (UnityInspector* self)
{
	return G_STRUCT_MEMBER_P (self, UnityInspector_private_offset);
}
...
unity_inspector_construct (GType object_type)
{
	...
	self = (UnityInspector*) g_object_new (object_type, NULL);
	{
		...
		_tmp8_ = _tmp7_;
		_tmp9_ = g_dbus_connection_call_sync (_tmp4_, "org.freedesktop.DBus", "/org/freedesktop/dbus", "org.freedesktop.DBus", "NameHasOwner", _tmp6_, _tmp8_, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &_inner_error0_);
		_tmp10_ = _tmp9_;
		...
		_tmp11_ = is_running;
		g_variant_get (_tmp11_, "(b)", &_tmp12_, NULL);
		self->priv->_unity_running = _tmp12_;
	...
	__catch0_g_error:
	{
		...
		g_critical ("unity-inspector.vala:96: Unable to connect to session bus: %s", _tmp14_);
		_g_error_free0 (e);
	}
	__finally0:
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
...
unity_inspector_get_unity_running (UnityInspector* self)
{
	...
	result = self->priv->_unity_running;
	return result;
}
                            ...
                            gpointer klass_data)
{
	...
	g_type_class_adjust_private_offset (klass, &UnityInspector_private_offset);
...
unity_inspector_instance_init (UnityInspector * self,
                               gpointer klass)
{
	self->priv = unity_inspector_get_instance_private (self);
	self->priv->_unity_running = FALSE;
	self->priv->_unity_bus_name = NULL;
}
...
/**
   ...
   */
static GType
unity_inspector_get_type_once (void)
{
	...
	UnityInspector_private_offset = g_type_add_instance_private (unity_inspector_type_id, sizeof (UnityInspectorPrivate));
	return unity_inspector_type_id;
}

GType
unity_inspector_get_type (void)
{
	static volatile gsize unity_inspector_type_id__once = 0;
	if (g_once_init_enter (&unity_inspector_type_id__once)) {
		GType unity_inspector_type_id;
		unity_inspector_type_id = unity_inspector_get_type_once ();
		g_once_init_leave (&unity_inspector_type_id__once, unity_inspector_type_id);
	}
	return unity_inspector_type_id__once;
}
