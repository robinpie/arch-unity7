/* unity-scope-tracker.c generated by valac 0.56.19, the Vala compiler
...
struct _UnityInternalScopeTrackerInitChannelData {
	...
	GHashTable* _tmp72_;
	UnityProtocolScopeProxy* _tmp73_;
	gchar* _tmp74_;
	UnityInternalUtilsAsyncOnce* _tmp75_;
	const gchar* _tmp76_;
	const gchar* scope_channel_id;
	UnityInternalUtilsAsyncOnce* _tmp77_;
	gconstpointer _tmp78_;
	const gchar* _tmp79_;
	UnityInternalUtilsAsyncOnce* _tmp80_;
	const gchar* _tmp81_;
	GError* _inner_error0_;
};
...
struct _UnityInternalScopeTrackerSearchWrapperData {
	...
	UnityInternalResultsSynchronizer* synchronizer;
	const gchar* _tmp46_;
	UnityInternalResultsSynchronizer* _tmp47_;
	UnityInternalResultsSynchronizer* _tmp48_;
	UnityInternalResultsSynchronizer* _tmp49_;
	UnityInternalResultsSynchronizer* _tmp50_;
	...
	GVariant* _tmp61_;
	GVariant* _tmp62_;
	UnityInternalResultsSynchronizer* _tmp63_;
	GError* _inner_error0_;
};
...
struct _UnityInternalScopeTrackerPushWrapperData {
	...
	UnityInternalResultsSynchronizer* _tmp32_;
	GError* _inner_error0_;
};

static gint UnityInternalScopeTracker_private_offset;
...
static GType unity_internal_scope_tracker_get_type_once (void);
...
static GType
unity_internal_channel_update_flags_get_type_once (void)
{
	...
	return unity_internal_channel_update_flags_type_id;
}

GType
unity_internal_channel_update_flags_get_type (void)
{
	static volatile gsize unity_internal_channel_update_flags_type_id__once = 0;
	if (g_once_init_enter (&unity_internal_channel_update_flags_type_id__once)) {
		GType unity_internal_channel_update_flags_type_id;
		unity_internal_channel_update_flags_type_id = unity_internal_channel_update_flags_get_type_once ();
		g_once_init_leave (&unity_internal_channel_update_flags_type_id__once, unity_internal_channel_update_flags_type_id);
	}
	return unity_internal_channel_update_flags_type_id__once;
}

static inline gpointer
unity_internal_scope_tracker_get_instance_private (UnityInternalScopeTracker* self)
{
	return G_STRUCT_MEMBER_P (self, UnityInternalScopeTracker_private_offset);
}
                                                     ...
                                                     UnityProtocolScopeProxy* proxy)
{
	...
	while (TRUE) {
		...
		val = (UnityInternalUtilsAsyncOnce*) _tmp2_;
		if (!_tmp3_) {
			break;
		}
		_tmp4_ = val;
                                               ...
                                               const gchar* channel_id)
{
	...
	UnityInternalResultsSynchronizer* result;
	g_return_val_if_fail (self != NULL, NULL);
                                              ...
                                              UnityProtocolScopeRegistryScopeMetadata* metadata)
{
	...
	pref_man = _tmp0_;
	if (metadata->remote_content) {
		UnityPreferencesManager* _tmp2_;
		UnityPreferencesManagerRemoteContent _tmp3_;
		UnityPreferencesManagerRemoteContent _tmp4_;
		_tmp2_ = pref_man;
		_tmp3_ = unity_preferences_manager_get_remote_content_search (_tmp2_);
		_tmp4_ = _tmp3_;
		_tmp1_ = _tmp4_ == UNITY_PREFERENCES_MANAGER_REMOTE_CONTENT_NONE;
	} else {
		_tmp1_ = FALSE;
	}
	if (_tmp1_) {
                                             ...
                                             GError** error)
{
	...
	if (_tmp1_) {
		GError* _tmp2_;
		_tmp2_ = g_error_new_literal (UNITY_PROTOCOL_SCOPE_ERROR, UNITY_PROTOCOL_SCOPE_ERROR_DISABLED_CONTENT, "Global search is disabled");
	...
	if (_tmp3_) {
		...
		_tmp6_ = self->priv->binary_present;
		_tmp7_ = metadata->query_binary;
		if (!g_hash_table_contains (_tmp6_, _tmp7_)) {
			...
			gchar* _tmp12_;
			gchar* _tmp13_;
			_tmp8_ = self->priv->binary_present;
			_tmp9_ = metadata->query_binary;
			_tmp10_ = g_strdup (_tmp9_);
			_tmp11_ = metadata->query_binary;
			_tmp12_ = g_find_program_in_path (_tmp11_);
			_tmp13_ = _tmp12_;
			g_hash_table_insert (_tmp8_, _tmp10_, (gpointer) ((gintptr) (_tmp13_ != NULL)));
			_g_free0 (_tmp13_);
		}
		_tmp14_ = self->priv->binary_present;
		_tmp15_ = metadata->query_binary;
		_tmp16_ = g_hash_table_lookup (_tmp14_, _tmp15_);
		if (!((gboolean) ((gintptr) _tmp16_))) {
			GError* _tmp17_;
			_tmp17_ = g_error_new_literal (UNITY_PROTOCOL_SCOPE_ERROR, UNITY_PROTOCOL_SCOPE_ERROR_DISABLED_CONTENT, "Required application isn't installed");
...
unity_internal_scope_tracker_create_proxy_co (UnityInternalScopeTrackerCreateProxyData* _data_)
{
	...
	return FALSE;
	_state_1:
	_data_->_tmp0_ = unity_protocol_scope_proxy_new_from_metadata_finish (_data_->_res_, &_data_->_inner_error0_);
	...
	g_object_set (_data_->_tmp3_, "auto-reconnect", FALSE, NULL);
	_data_->_tmp4_ = _data_->proxy;
	unity_internal_scope_tracker_check_proxy_vs_metadata (_data_->self, _data_->_tmp4_, _data_->metadata, &_data_->_inner_error0_);
	if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
		g_task_return_error (_data_->_async_result, _data_->_inner_error0_);
                                                   ...
                                                   UnityProtocolScopeProxy* proxy)
{
		...
		for (channel_key_it = channel_key_collection; channel_key_it != NULL; channel_key_it = channel_key_it->next) {
			...
			channel_key = (const gchar*) channel_key_it->data;
			{
				...
				if (g_str_has_prefix (_tmp5_, _tmp6_)) {
					...
					_vala_array_add12 (&invalid_keys, &invalid_keys_length1, &_invalid_keys_size_, _tmp8_);
				}
			}
		}
	...
	_tmp9_ = invalid_keys;
	_tmp9__length1 = invalid_keys_length1;
	{
		...
		channel_key_collection = _tmp9_;
		channel_key_collection_length1 = _tmp9__length1;
		for (channel_key_it = 0; channel_key_it < channel_key_collection_length1; channel_key_it = channel_key_it + 1) {
			const gchar* channel_key = NULL;
			channel_key = channel_key_collection[channel_key_it];
			{
				...
				GHashTable* _tmp12_;
				const gchar* _tmp13_;
				_tmp10_ = self->priv->scope_channel_ids;
				_tmp11_ = channel_key;
				g_hash_table_remove (_tmp10_, _tmp11_);
				_tmp12_ = self->priv->scope_models;
				_tmp13_ = channel_key;
                                                     ...
                                                     UnityProtocolChannelType channel_type)
{
	...
	_g_free0 (_tmp2_);
	flags = _tmp4_;
	if (flags == UNITY_INTERNAL_CHANNEL_UPDATE_FLAGS_NONE) {
		return;
	}
	g_signal_emit (self, unity_internal_scope_tracker_signals[UNITY_INTERNAL_SCOPE_TRACKER_RESULTS_INVALIDATED_SIGNAL], 0, flags);
}
                                                      ...
                                                      GError** error)
{
	gboolean _tmp0_;
	gboolean _tmp1_;
	UnityProtocolMetaDataSchemaInfo* _tmp7_;
	GError* _inner_error0_ = NULL;
	g_return_if_fail (self != NULL);
	g_return_if_fail (proxy != NULL);
	g_return_if_fail (metadata != NULL);
	...
	if (_tmp1_ != metadata->is_master) {
		...
		GError* _tmp6_;
		_tmp2_ = metadata->id;
		_tmp3_ = g_strdup_printf ("Scope file info for '%s' doesn't match on IsMaster key", _tmp2_);
		_tmp4_ = _tmp3_;
		_tmp5_ = g_error_new_literal (UNITY_PROTOCOL_SCOPE_ERROR, UNITY_PROTOCOL_SCOPE_ERROR_DATA_MISMATCH, _tmp4_);
		_tmp6_ = _tmp5_;
		_g_free0 (_tmp4_);
	...
	_tmp7_ = metadata->required_metadata;
	if (_tmp7_ != NULL) {
		...
		_tmp9_ = unity_protocol_meta_data_schema_info_as_hash_table (_tmp8_);
		dict = _tmp9_;
		_tmp10_ = dict;
		...
		while (TRUE) {
			...
			field_name = (const gchar*) _tmp11_;
			schema = (const gchar*) _tmp12_;
			if (!_tmp13_) {
				break;
			}
			_tmp14_ = unity_protocol_scope_proxy_get_metadata (proxy);
			_tmp15_ = _tmp14_;
			_tmp16_ = field_name;
			_tmp17_ = g_hash_table_lookup (_tmp15_, _tmp16_);
			_tmp18_ = schema;
			if (g_strcmp0 ((const gchar*) _tmp17_, _tmp18_) != 0) {
				const gchar* _tmp19_;
				gchar* _tmp20_;
				gchar* _tmp21_;
				GError* _tmp22_;
				GError* _tmp23_;
				_tmp19_ = metadata->id;
				_tmp20_ = g_strdup_printf ("Scope file info for '%s' doesn't match on RequiredMetadata key", _tmp19_);
				_tmp21_ = _tmp20_;
				_tmp22_ = g_error_new_literal (UNITY_PROTOCOL_SCOPE_ERROR, UNITY_PROTOCOL_SCOPE_ERROR_DATA_MISMATCH, _tmp21_);
				_tmp23_ = _tmp22_;
				_g_free0 (_tmp21_);
                                                 ...
                                                 const gchar* master_channel_id)
{
	...
	if (_tmp3_ != NULL) {
		...
		_tmp6_ = _tmp5_;
		g_object_set_qdata_full ((GObject*) _tmp6_, unity_internal_scope_tracker_DEDUP_MODEL_QUARK, NULL, g_object_unref);
		_tmp7_ = self->priv->synchronizers;
		g_hash_table_remove (_tmp7_, master_channel_id);
	}
	_tmp8_ = g_str_hash;
	_tmp9_ = g_str_equal;
	_tmp10_ = g_hash_table_new_full (_tmp8_, _tmp9_, _g_free0_, _g_object_unref0_);
	channel_keys_to_close = _tmp10_;
	{
		gint i = 0;
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp11_ = FALSE;
				_tmp13_ = self->priv->scope_proxy_arr;
				_tmp14_ = vala_g_ptr_array_get_length (_tmp13_);
				_tmp15_ = _tmp14_;
				if (!(i < _tmp15_)) {
					break;
				}
				_tmp16_ = self->priv->scope_proxy_arr;
				_tmp17_ = g_ptr_array_index (_tmp16_, (guint) i);
				_tmp18_ = unity_internal_scope_tracker_get_channel_key (self, master_channel_id, (UnityProtocolScopeProxy*) _tmp17_);
				channel_key = _tmp18_;
				_tmp19_ = self->priv->scope_channel_ids;
				_tmp20_ = channel_key;
				if (g_hash_table_contains (_tmp19_, _tmp20_)) {
					...
					_tmp21_ = self->priv->scope_channel_ids;
					_tmp22_ = channel_key;
					_tmp23_ = g_hash_table_lookup (_tmp21_, _tmp22_);
					_tmp24_ = unity_internal_utils_async_once_get_data ((UnityInternalUtilsAsyncOnce*) _tmp23_);
					if (((const gchar*) _tmp24_) != NULL) {
						GHashTable* _tmp25_;
						const gchar* _tmp26_;
						gchar* _tmp27_;
						GPtrArray* _tmp28_;
						gconstpointer _tmp29_;
						UnityProtocolScopeProxy* _tmp30_;
						_tmp25_ = channel_keys_to_close;
						_tmp26_ = channel_key;
						_tmp27_ = g_strdup (_tmp26_);
						_tmp28_ = self->priv->scope_proxy_arr;
						_tmp29_ = g_ptr_array_index (_tmp28_, (guint) i);
						_tmp30_ = _g_object_ref0 ((UnityProtocolScopeProxy*) _tmp29_);
						g_hash_table_insert (_tmp25_, _tmp27_, _tmp30_);
					}
				}
	...
	_tmp31_ = channel_keys_to_close;
	g_hash_table_iter_init (&iter, _tmp31_);
	while (TRUE) {
		...
		gchar* _tmp39_;
		const gchar* _tmp40_;
		UnityProtocolScopeProxy* _tmp41_;
		const gchar* _tmp42_;
		GHashTable* _tmp43_;
		const gchar* _tmp44_;
		GHashTable* _tmp45_;
		const gchar* _tmp46_;
		_tmp34_ = g_hash_table_iter_next (&iter, (gpointer*) (&_tmp32_), (gpointer*) (&_tmp33_));
		channel_key = (const gchar*) _tmp32_;
		proxy = (UnityProtocolScopeProxy*) _tmp33_;
		if (!_tmp34_) {
			break;
		}
		_tmp35_ = self->priv->scope_channel_ids;
		_tmp36_ = channel_key;
		_tmp37_ = g_hash_table_lookup (_tmp35_, _tmp36_);
		_tmp38_ = unity_internal_utils_async_once_get_data ((UnityInternalUtilsAsyncOnce*) _tmp37_);
		_tmp39_ = g_strdup ((const gchar*) _tmp38_);
		child_channel_id = _tmp39_;
		_tmp40_ = child_channel_id;
		if (_tmp40_ == NULL) {
			_g_free0 (child_channel_id);
			continue;
		}
		_tmp41_ = proxy;
		_tmp42_ = child_channel_id;
		unity_protocol_scope_proxy_close_channel (_tmp41_, _tmp42_, NULL, NULL, NULL);
		_tmp43_ = self->priv->scope_channel_ids;
		_tmp44_ = channel_key;
		g_hash_table_remove (_tmp43_, _tmp44_);
		_tmp45_ = self->priv->scope_models;
		_tmp46_ = channel_key;
...
unity_internal_scope_tracker_wait_for_seqnum_co (UnityInternalScopeTrackerWaitForSeqnumData* _data_)
{
	...
	_state_1:
	;
	g_signal_handler_disconnect (_data_->model, _data_->update_sig_id);
	if (_data_->_data25_->src_id != ((guint) 0)) {
		g_source_remove (_data_->_data25_->src_id);
	} else {
		_data_->_tmp1_ = g_error_new_literal (G_DBUS_ERROR, G_DBUS_ERROR_TIMEOUT, "Timed out waiting for model update");
                                             ...
                                             const gchar* scope_id)
{
	...
	UnityInternalUtilsAsyncOnce* result;
	g_return_val_if_fail (self != NULL, NULL);
	g_return_val_if_fail (scope_id != NULL, NULL);
	_tmp0_ = self->priv->scope_proxies;
                                           ...
                                           gpointer _user_data_)
{
	...
	gchar* _tmp2_;
	g_return_if_fail (self != NULL);
	g_return_if_fail (master_channel != NULL);
...
unity_internal_scope_tracker_init_channel_co (UnityInternalScopeTrackerInitChannelData* _data_)
{
	...
	if (g_hash_table_contains (_data_->_tmp1_, _data_->scope_id)) {
		...
		unity_internal_scope_tracker_perform_checks (_data_->self, (UnityProtocolScopeRegistryScopeMetadata*) _data_->_tmp3_, _data_->channel_type, &_data_->_inner_error0_);
		if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
			g_task_return_error (_data_->_async_result, _data_->_inner_error0_);
		...
		if (unity_internal_utils_async_once_enter_finish (_data_->_tmp5_, _data_->_res_)) {
			_data_->actual_proxy = NULL;
			{
				...
				if (!g_hash_table_contains (_data_->_tmp6_, _data_->scope_id)) {
					_data_->_tmp8_ = unity_protocol_scope_registry_scope_metadata_for_id (_data_->scope_id, &_data_->_inner_error0_);
				...
				unity_internal_scope_tracker_perform_checks (_data_->self, _data_->_tmp15_, _data_->channel_type, &_data_->_inner_error0_);
				if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
					_unity_protocol_scope_registry_scope_metadata_unref0 (_data_->metadata);
					goto __catch0_g_error;
				}
				...
				return FALSE;
				_state_2:
				_data_->_tmp18_ = unity_internal_scope_tracker_create_proxy_finish (_data_->self, _data_->_res_, &_data_->_inner_error0_);
	...
	if (!unity_internal_utils_async_once_is_initialized (_data_->_tmp51_)) {
		...
		if (unity_internal_utils_async_once_enter_finish (_data_->_tmp52_, _data_->_res_)) {
			_data_->chan_id = NULL;
			{
				...
				return FALSE;
				_state_4:
				_data_->_tmp56_ = unity_protocol_scope_proxy_open_channel_finish (_data_->_tmp54_, _data_->_res_, &_data_->_tmp55_, &_data_->_inner_error0_);
				_g_object_unref0 (_data_->model);
...
"hannel %s", _data_->_tmp70_);
				...
				if (_data_->master_channel->channel_type == UNITY_PROTOCOL_CHANNEL_TYPE_DEFAULT) {
				...
				_data_->_tmp72_ = _data_->self->priv->master_update_flags;
				_data_->_tmp73_ = _data_->proxy;
				_data_->_tmp74_ = unity_internal_scope_tracker_get_update_key (_data_->self, _data_->_tmp73_, _data_->channel_type);
				g_hash_table_insert (_data_->_tmp72_, _data_->_tmp74_, (gpointer) ((gintptr) _data_->flag));
				_g_object_unref0 (_data_->synchronizer);
				_g_free0 (_data_->_tmp53_);
			}
			__finally1:
			{
				_data_->_tmp75_ = _data_->channel_id_once;
				_data_->_tmp76_ = _data_->chan_id;
				unity_internal_utils_async_once_leave (_data_->_tmp75_, _data_->_tmp76_);
			}
			if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
				g_task_return_error (_data_->_async_result, _data_->_inner_error0_);
				_g_free0 (_data_->chan_id);
	...
	_data_->_tmp78_ = unity_internal_utils_async_once_get_data (_data_->_tmp77_);
	_data_->scope_channel_id = (const gchar*) _data_->_tmp78_;
	_data_->_tmp79_ = _data_->scope_channel_id;
	if (_data_->_tmp79_ == NULL) {
		_data_->_tmp80_ = _data_->channel_id_once;
		unity_internal_utils_async_once_reset (_data_->_tmp80_);
	}
	_data_->_tmp81_ = _data_->scope_channel_id;
	_data_->result = _data_->_tmp81_;
	_unity_internal_utils_async_once_unref0 (_data_->channel_id_once);
	_g_free0 (_data_->channel_key);
	_g_error_free0 (_data_->failure);
	_unity_internal_utils_async_once_unref0 (_data_->proxy_once);
...
unity_internal_scope_tracker_activate_wrapper_co (UnityInternalScopeTrackerActivateWrapperData* _data_)
{
	...
	_data_->_tmp2_ = NULL;
	_data_->_tmp3_ = unity_internal_scope_tracker_get_channel_id (_data_->self, _data_->_tmp1_, _data_->scope_id, &_data_->_tmp2_, &_data_->_inner_error0_);
	_g_object_unref0 (_data_->proxy);
	...
	if (_data_->_tmp4_ == NULL) {
		_data_->_tmp6_ = NULL;
		_data_->_state_ = 1;
		unity_internal_scope_tracker_init_channel (_data_->self, _data_->master_channel, _data_->scope_id, _data_->master_channel->channel_type, unity_internal_scope_tracker_activate_wrapper_ready, _data_);
		return FALSE;
		_state_1:
		_data_->_tmp7_ = unity_internal_scope_tracker_init_channel_finish (_data_->self, _data_->_res_, &_data_->_tmp6_, &_data_->_inner_error0_);
		_g_object_unref0 (_data_->proxy);
	...
	_data_->_tmp9_ = _data_->proxy;
	_data_->_tmp10_ = _data_->scope_channel_id;
	memset (&_data_->_tmp11_, 0, sizeof (UnityProtocolActivationReplyRaw));
	_data_->_state_ = 2;
	unity_protocol_scope_proxy_activate (_data_->_tmp9_, _data_->_tmp10_, _data_->result_arr, (gint) _data_->result_arr_length1, _data_->action, _data_->hints, _data_->cancellable, unity_internal_scope_tracker_activate_wrapper_ready, _data_);
	return FALSE;
	_state_2:
	unity_protocol_scope_proxy_activate_finish (_data_->_tmp9_, _data_->_res_, &_data_->_tmp11_, &_data_->_inner_error0_);
	...
	memset (&_data_->_tmp8_, 0, sizeof (UnityProtocolActivationReplyRaw));
	_data_->result = _data_->_tmp12_;
	unity_protocol_activation_reply_raw_destroy (&_data_->_tmp8_);
	_g_object_unref0 (_data_->proxy);
	_data_->result_arr = (_vala_array_free (_data_->result_arr, _data_->result_arr_length1, (GDestroyNotify) g_variant_unref), NULL);
	g_task_return_pointer (_data_->_async_result, _data_, NULL);
	if (_data_->_state_ != 0) {
...
unity_internal_scope_tracker_search_wrapper_co (UnityInternalScopeTrackerSearchWrapperData* _data_)
{
	...
	return FALSE;
	_state_2:
	_data_->_tmp28_ = unity_protocol_scope_proxy_search_finish (_data_->_tmp26_, _data_->_res_, &_data_->_inner_error0_);
	...
	while (TRUE) {
		...
		_data_->key = (const gchar*) _data_->_tmp30_;
		_data_->variant = (GVariant*) _data_->_tmp31_;
		if (!_data_->_tmp32_) {
			break;
		}
		...
		if (g_strcmp0 (_data_->_tmp33_, UNITY_INTERNAL_SEARCH_SEQNUM_HINT) == 0) {
			...
			if (dee_serializable_model_get_seqnum (_data_->_tmp39_) < _data_->seqnum) {
				_data_->_tmp40_ = _data_->model;
				_data_->_state_ = 3;
				unity_internal_scope_tracker_wait_for_seqnum (_data_->self, G_TYPE_CHECK_INSTANCE_TYPE (_data_->_tmp40_, dee_shared_model_get_type ()) ? ((DeeSharedModel*) _data_->_tmp40_) : NULL, _data_->seqnum, unity_internal_scope_tracker_search_wrapper_ready, _data_);
				return FALSE;
				_state_3:
				unity_internal_scope_tracker_wait_for_seqnum_finish (_data_->self, _data_->_res_, &_data_->_inner_error0_);
				if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
					g_task_return_error (_data_->_async_result, _data_->_inner_error0_);
					_g_object_unref0 (_data_->model);
			...
			_data_->_tmp41_ = _data_->self->priv->scope_models;
			_data_->_tmp42_ = _data_->channel_key;
			_data_->_tmp43_ = g_hash_table_lookup (_data_->_tmp41_, _data_->_tmp42_);
			_data_->_tmp44_ = _data_->model;
			if (((DeeSerializableModel*) _data_->_tmp43_) != _data_->_tmp44_) {
				_data_->result = _data_->reply_hints;
				_g_object_unref0 (_data_->model);
				_g_hash_table_unref0 (_data_->reply_dict);
				_g_object_unref0 (_data_->sync);
			...
			if (_data_->seqnum == _data_->last_seq_num) {
				_data_->_tmp45_ = _data_->channel_key;
				g_debug ("unity-scope-tracker.vala:558: Model seqnum for channel key %s not chan" \
"ged, copying", _data_->_tmp45_);
				_data_->_tmp46_ = _data_->master_channel->id;
				_data_->_tmp47_ = unity_internal_scope_tracker_get_synchronizer (_data_->self, _data_->_tmp46_);
				_data_->_tmp48_ = _g_object_ref0 (_data_->_tmp47_);
				_data_->synchronizer = _data_->_tmp48_;
				_data_->_tmp49_ = _data_->synchronizer;
				if (_data_->_tmp49_ != NULL) {
					_data_->_tmp50_ = _data_->synchronizer;
					_data_->_tmp51_ = _data_->model;
					unity_internal_results_synchronizer_copy_model (_data_->_tmp50_, (DeeModel*) _data_->_tmp51_);
				} else {
					_data_->_tmp52_ = _data_->master_channel->id;
					g_warning ("unity-scope-tracker.vala:563: No synchronizer for master channel %s", _data_->_tmp52_);
				}
				_g_object_unref0 (_data_->synchronizer);
			}
			_g_object_unref0 (_data_->model);
		} else {
			_data_->_tmp53_ = _data_->key;
			if (g_strcmp0 (_data_->_tmp53_, UNITY_INTERNAL_SEARCH_TIME_HINT) == 0) {
				_data_->_tmp54_ = _data_->reply_hints;
				_data_->_tmp55_ = g_strdup_printf ("%s:%s", UNITY_INTERNAL_SEARCH_TIME_HINT, _data_->scope_id);
				_data_->_tmp56_ = _data_->variant;
				_data_->_tmp57_ = _g_variant_ref0 (_data_->_tmp56_);
				g_hash_table_insert (_data_->_tmp54_, _data_->_tmp55_, _data_->_tmp57_);
			} else {
				_data_->_tmp58_ = _data_->reply_hints;
				_data_->_tmp59_ = _data_->key;
				_data_->_tmp60_ = g_strdup (_data_->_tmp59_);
				_data_->_tmp61_ = _data_->variant;
	...
	_data_->_tmp63_ = _data_->sync;
	unity_internal_results_synchronizer_disable_provider (_data_->_tmp63_, _data_->scope_id);
	_data_->result = _data_->reply_hints;
	_g_hash_table_unref0 (_data_->reply_dict);
	_g_object_unref0 (_data_->sync);
	_g_free0 (_data_->channel_key);
                                           ...
                                           gpointer _user_data_)
{
	...
	GCancellable* _tmp6_;
	g_return_if_fail (self != NULL);
	g_return_if_fail (parent_channel != NULL);
	g_return_if_fail (search_string != NULL);
	g_return_if_fail (master_scope_id != NULL);
	g_return_if_fail (scope_id != NULL);
...
unity_internal_scope_tracker_push_wrapper_co (UnityInternalScopeTrackerPushWrapperData* _data_)
{
	...
	_data_->_tmp2_ = NULL;
	_data_->_tmp3_ = unity_internal_scope_tracker_get_channel_id (_data_->self, _data_->_tmp1_, _data_->master_scope_id, &_data_->_tmp2_, &_data_->_inner_error0_);
	_g_object_unref0 (_data_->proxy);
	_data_->proxy = _data_->_tmp2_;
	_data_->_tmp0_ = _data_->_tmp3_;
	if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
	...
	_data_->_state_ = 2;
	unity_protocol_scope_proxy_push_results (_data_->_tmp17_, _data_->_tmp18_, _data_->search_string, _data_->scope_id, _data_->results_model, _data_->categories, (gint) _data_->categories_length1, _data_->cancellable, unity_internal_scope_tracker_push_wrapper_ready, _data_);
	return FALSE;
	_state_2:
	_data_->_tmp19_ = unity_protocol_scope_proxy_push_results_finish (_data_->_tmp17_, _data_->_res_, &_data_->_inner_error0_);
	...
	while (TRUE) {
		...
		_data_->key = (const gchar*) _data_->_tmp21_;
		_data_->variant = (GVariant*) _data_->_tmp22_;
		if (!_data_->_tmp23_) {
			break;
		}
		...
		if (g_strcmp0 (_data_->_tmp24_, UNITY_INTERNAL_SEARCH_SEQNUM_HINT) == 0) {
			...
			if (dee_serializable_model_get_seqnum (_data_->_tmp30_) < _data_->seqnum) {
				_data_->_tmp31_ = _data_->model;
				_data_->_state_ = 3;
				unity_internal_scope_tracker_wait_for_seqnum (_data_->self, G_TYPE_CHECK_INSTANCE_TYPE (_data_->_tmp31_, dee_shared_model_get_type ()) ? ((DeeSharedModel*) _data_->_tmp31_) : NULL, _data_->seqnum, unity_internal_scope_tracker_push_wrapper_ready, _data_);
				return FALSE;
				_state_3:
				unity_internal_scope_tracker_wait_for_seqnum_finish (_data_->self, _data_->_res_, &_data_->_inner_error0_);
				if (G_UNLIKELY (_data_->_inner_error0_ != NULL)) {
	...
	_data_->_tmp32_ = _data_->sync;
	unity_internal_results_synchronizer_disable_provider (_data_->_tmp32_, _data_->master_scope_id);
	_g_hash_table_unref0 (_data_->reply_dict);
	_g_object_unref0 (_data_->sync);
	_g_free0 (_data_->channel_key);
	_g_object_unref0 (_data_->proxy);
                                         ...
                                         gpointer klass_data)
{
	...
	g_type_class_adjust_private_offset (klass, &UnityInternalScopeTracker_private_offset);
...
unity_internal_scope_tracker_instance_init (UnityInternalScopeTracker * self,
                                            gpointer klass)
{
	self->priv = unity_internal_scope_tracker_get_instance_private (self);
}
...
static GType
unity_internal_scope_tracker_get_type_once (void)
{
	...
	UnityInternalScopeTracker_private_offset = g_type_add_instance_private (unity_internal_scope_tracker_type_id, sizeof (UnityInternalScopeTrackerPrivate));
	return unity_internal_scope_tracker_type_id;
}

GType
unity_internal_scope_tracker_get_type (void)
{
	static volatile gsize unity_internal_scope_tracker_type_id__once = 0;
	if (g_once_init_enter (&unity_internal_scope_tracker_type_id__once)) {
		GType unity_internal_scope_tracker_type_id;
		unity_internal_scope_tracker_type_id = unity_internal_scope_tracker_get_type_once ();
		g_once_init_leave (&unity_internal_scope_tracker_type_id__once, unity_internal_scope_tracker_type_id);
	}
	return unity_internal_scope_tracker_type_id__once;
}
