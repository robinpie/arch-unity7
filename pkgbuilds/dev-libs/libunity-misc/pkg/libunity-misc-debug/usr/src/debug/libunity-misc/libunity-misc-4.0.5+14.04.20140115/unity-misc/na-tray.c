            ...
            TraysScreen   *trays_screen)
{
  ...
  if (priv->cb && !priv->cb (tray, icon, priv->data))
    {
      return;
    }
                        ...
                        gpointer             data)
{
  ...
  tray = g_object_new (NA_TYPE_TRAY,
		       "screen", screen,
...
NaTrayManager *
na_tray_get_manager (NaTray *tray)
{
  return trays_screens [gdk_screen_get_number (tray->priv->screen)].tray_manager;
}
